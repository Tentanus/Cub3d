...
static bool	set_infovalue(t_cub3d *info, t_type_id id, char *str, ssize_t *idx)
{
	...
	{
		if (id == TYPE_ID_NORTH)
			info->text_no = get_texture(str, idx, &err);
		else if (id == TYPE_ID_SOUTH)
			info->text_so = get_texture(str, idx, &err);
		else if (id == TYPE_ID_WEST)
			info->text_we = get_texture(str, idx, &err);
		else if (id == TYPE_ID_EAST)
			info->text_ea = get_texture(str, idx, &err);
		else if (id == TYPE_ID_FLOOR)
			info->col_fl = get_colour(str, idx, &err);
		else if (id == TYPE_ID_CEILING)
			info->col_ce = get_colour(str, idx, &err);
	}
	cbd_error(err);
	return (err);
}
